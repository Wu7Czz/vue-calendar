    ...
    <div class="calendar">
        ...
        <tr v-for="(day,k1) in days" style="{'animation-delay',(k1*30)+'ms'}">
            <td v-for="(child,k2) in day" :class="{'selected':child.selected,'disabled':child.disabled}" @click="select(k1,k2,$event)">
                ...
                <div class="text" v-if="child.eventName!=undefined" v-html="child.eventName"></div>
            </td>
        </tr>
        </tbody>
        </table>
...
export default {
    ...
    watch:{
        ...
        begin(){
            this.isInit && this.render(this.year,this.month)
        },
        end(){
            this.isInit && this.render(this.year,this.month)
        },
    ...
    methods: {
        ...
        render(y, m) {
            ...
            for (i = 1; i <= lastDateOfMonth; i++) {
                ...
                if (this.range) { // 范围
                    ...
                    options.disabled = this.handleDateDisabled(i)
                    ...
                    if(this.value.filter(v => {return this.year === v[0] && this.month === v[1]-1 && i === v[2] }).length>0 ){
                        options = Object.assign({day: i,selected:true},this.getEvents(this.year,this.month+1,i))
                    }else{
                        options = Object.assign({day: i,selected:false},this.getEvents(this.year,this.month+1,i))
                        options.disabled = this.handleDateDisabled(i)
                    }
        ...
        handleDateDisabled(i) {
            var disabled = false
            if (this.begin.length>0) {
                let beginTime = Number(new Date(parseInt(this.begin[0]),parseInt(this.begin[1]) - 1,parseInt(this.begin[2])))
                if (beginTime > Number(new Date(this.year, this.month, i))) disabled = true
            }
            if (this.end.length>0 && !disabled){
                let endTime = Number(new Date(parseInt(this.end[0]),parseInt(this.end[1]) - 1,parseInt(this.end[2])))
                if (endTime <  Number(new Date(this.year, this.month, i))) disabled = true
            }
            if (this.disabled.length>0 && !disabled){
            ...
            //     const v = this.minDate || [0,0,0]
            //     this.year === v[0] && this.month === v[1]-1 && i === v[2] 
            //     if (this.year > v[0]){
            //         options.disabled = true
            //     } else if (this.month > v[1]-1) {
            //         options.disabled = true
            //     } else if (i > v[2]){
            //         options.disabled = true
            //     }
        ...
        getEvents(y,m,d){
            ...
            if(eventName!=undefined){
                data.eventName=eventName.replace("script","")
            }
            return data
        },
        // 上月
